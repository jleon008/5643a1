...
public class ParticleSystemBuilder implements GLEventListener {
	...
	public boolean exportRenders = false;
	
	public boolean goo_on = false;
	...
	// KeyListener
	{
		...
		void simulateAndDisplayScene(GL gl) {
			...
			if (simulate) {
				...
				if (goo_on && frameNumber % (Math.pow(2, 1 + frameNumber / 100)) == 0) {
					addGoo(frameNumber);
				}
				
				if (povExport && exportRenders) {
		...
		private void addGoo(int n) {
			...
			for (int x = 0; x < 2; x++) {
				for (int y = 0; y < 1; y++) {
					for (int z = 0; z < 2; z++) {
						float f = (n % 10) / 100;
						Particle p = PS.createGooParticle(new Point3d(
								.4 + x * 0.03 + f * .1,
								.7 + (x ^ z) * 0.03 + f * .02,
								.3 + z * 0.03 + f * .1));
						p.v = new Vector3d(0, -0.4, 0);
					}
				}
			}
		}
					...
					for (int i = 0; i < size; i++) {
						for (int j = 0; j < size; j++) {
							paper[i][j] = PS.createPaperParticle(new Point3d(
									.3 + kerran * .1 + (i + 0.5) * spacing + (r.nextFloat() - 1) * .00005,
									0.5 - kerran * .1 - i * 0.0025 + (r.nextFloat() - 1) * .00005,
									.2 + (j + 0.5) * spacing + (r.nextFloat() - 1) * .00005));
						}
					}
					// connect them
					for (int i = 0; i < size; i++) {
	...
	public static void main(String[] args) {
		try {
			...
			if (args.length != 0) {
				psb.povExport = true; //also have to turn on render exports by pressing 'r'
